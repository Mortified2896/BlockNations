...
public static class AITurnLogic
{
        ...
        bool aiHasPerfectInfo)
    {
        ...
        bool visibleFastThreatNearCity = false;
        ...
        for (int i = 0; i < visibleEnemyFastUnits.Count; i++)
        {
            ...
            {
                visibleFastThreatNearCity = true;
                break;
            }
        }
        ...
        else
        {
            plan.AssignedCombatUnit = FindNearestUnitForPosition(
                ...
                city,
                unitToSkip: cityOccupantCanBeCombatReserve ? cityOccupant : null);
        }

        if (!plan.CanVacateCityTile && TryFindScoutScreeningTile(gridManager, city, defenseTile, threatTiles, visibleTiles, out TileVisibility scoutTile))
        {
